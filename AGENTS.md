...
Be mindful of the user's `gh` CLI API rate limit — batch requests where possible and avoid unnecessary calls. All code, commands, and scripts must be compatible with macOS, Linux, and Windows. Factory jobs use Hetzner fast runners only when real-run p90 is under about two minutes; receipt-only resumes do not count. `df-runner-1-1` is dedicated fast (never heavy); `df-runner-2-1`, `2-2`, `3-1`, and `3-2` carry both fast and heavy for overflow; `df-runner-1-2`, `4-1`, and `4-2` are heavy-only: five fast and seven heavy across eight registrations. Intake, builds, reviews, rework, source QA, gates and unmeasured Plan/Plan-check keep their heavy selectors. Mac-only work stays on M4. Fitness Coach alone reconciles these fixed labels from an independent M4 maintenance job. An offline dedicated runner uses the four shared fast targets, without promoting a new dedicated runner or cancelling busy work. Recovery requires at least one fast target online and explicit online/offline status for all eight pilot identities; unknown/missing status or incomplete identity inventory refuses writes. Required labels are added and read back before extras are removed; partial writes converge on the next heartbeat. Unrelated runners and labels are preserved.
...
Standing rulings move between jobs as a UTF-8 artifact; processes receive only `STANDING_RULINGS_FILE`, never the uncapped contents in an environment variable or argument.
